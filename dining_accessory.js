...
// also add an "optional" Characteristic for saturdation
light
  .getService(Service.Lightbulb)
...
noble.on("discover", function(peripheral) {
  ...
  if(perifSel==null){
     console.log(timestamp()+"found "+peripheral.uuid);
     if((peripheral.uuid==uuidMyLamp)||(uuidMyLamp==null)){
	perifSel=peripheral;
	console.log(timestamp()+"uuid matches!|state: "+perifSel.state);
        ...
        perifSel.connect(function(error) {
       	  ...
       	  bulb.connect();
       	  console.log(timestamp()+"state: " + perifSel.state + " / " + bulb.connected);
        });
     }
  // De ahí en adelante tan solo se reconecta la luz y ya conecta el dispositivo al hacerlo
  }else{
      console.log(timestamp()+ perifSel.state);
  }
